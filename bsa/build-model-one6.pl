...
open(my $in1, "<:encoding(UTF-8)", "$sent_file_1.words") or
...
while (1) {
	...
	if ($iteration_count > 1 && $score_sum >= $prev_score_sum) {
...
open(my $out, ">:encoding(UTF-8)", "$sent_file_1.$sent_file_2_mod.model-one") or die "Failed to create '$sent_file_1.$sent_file_2_mod.model-one'!\n$!";
